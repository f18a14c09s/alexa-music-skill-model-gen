...
public class AskMusicMessageUnitTestGenerationCLI {
...
//        inferClasses().entrySet().forEach(clazz -> System.out.printf("%s:%n\t%s%n", clazz.getKey(), clazz.getValue()));
    ...
    private static Map<String, String> inferClasses() throws IOException {
        ...
        new AskMusicApiDocAnalyzer().getMessageModel().forEach(messageInfo -> {
            try {
                retval.putAll(new AskMusicModelUnitTestJavaGenerator().generateTestClasses(packageName,
